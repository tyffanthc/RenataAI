...
class SpanshPlannerBase(ttk.Frame):
    ...
    def _copy_indices_delimited_to_clipboard(
        ...
        *,
        ...
        lines: list[str] = []
        if include_header:
            header = common.format_header_delimited(self._schema_id, sep)
            if header:
                lines.append(header)
        for row in rows:
            line = common.format_row_delimited(self._schema_id, row, sep)
            if line:
                lines.append(line)
    ...
    def _copy_clicked_delimited(self, payload: dict[str, Any], *, sep: str, include_header: bool, context: str) -> None:
        idx = int(payload.get("row_index", -1))
        if idx < 0 or idx >= len(self._results_rows):
            return
    ...
    def _copy_selected_delimited(self, payload: dict[str, Any], *, sep: str, include_header: bool, context: str) -> None:
        indices = self._selected_internal_indices()
        if not indices:
            idx = int(payload.get("row_index", -1))
            if idx >= 0:
