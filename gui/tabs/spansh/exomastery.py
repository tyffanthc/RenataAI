...
class ExomasteryTab(SpanshPlannerBase):
    ...
    def _build_ui(self):
        ...
        layout.add_labeled_entry(
            f_extra,
            0,
            ui.LABEL_MIN_LANDMARK_VALUE,
            self.var_min_landmark,
        )
        ...
        if self._use_treeview:
            self.lst = common.stworz_tabele_trasy(fr, title=ui.LIST_TITLE_EXOMASTERY)
        else:
            self.lst = common.stworz_liste_trasy(fr, title=ui.LIST_TITLE_EXOMASTERY)
